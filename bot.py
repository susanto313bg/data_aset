...
# Konversi ADMIN_ID dari environment variable ke integer secara aman
try:
    ADMIN_ID = int(os.getenv("ADMIN_ID", "8617408271").strip())
except ValueError:
    ADMIN_ID = 8617408271
...
# === SYSTEM STATES ===
# Set default ke True agar saat pertama kali jalan bot langsung bisa diakses
PUBLIC_ACCESS = True 

# Pesan Textbox Manual (Bisa diubah secara bebas oleh Admin)
MAINTENANCE_TEXT = (
...
# Render Menu Dashboard Admin (3 Tombol Utama)
...
# Fungsi Mengirim Pesan Penolakan ke Pengguna Biasa
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user_id):
        pesan = (
            ...
            f"• Ketik angka <b>TERM ID</b> untuk mencari data langsung."
        )
        await update.message.reply_text(pesan, parse_mode="HTML")
        return

    # Jika BUKAN Admin dan Bot dalam Mode Privat
    if not PUBLIC_ACCESS:
        await send_access_denied_msg(update)
        return

    # User Biasa saat Mode Public
    msg = (
...
# Callback Handler Tombol Menu Admin
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. ADMIN - Input Teks Maintenance Baru
    ...
    # 2. ADMIN - Membuka Panel Kontrol dengan ketik "edit" atau "/edit"
    ...
    # 3. PENGECEKAN HAK AKSES USER BIASA
    # Jika BUKAN Admin DAN status bot PRIVAT -> Tolak Akses!
    ...
    # 4. PENCARIAN DATA EXCEL (Bisa dilakukan Admin kapan saja & User saat Public)
