...
from zoneinfo import ZoneInfo  # Menggunakan modul bawaan Python
...
def get_data_by_term_id(tid_input: str):
    ...
    try:
        ...
        # Konversi ke UPPERCASE agar fleksibel huruf besar/kecil
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Format Waktu Real-time WIB tanpa library eksternal pytz
    ...
    nama_hari = hari_indo.get(sekarang.strftime('%A'), sekarang.strftime('%A'))
    nama_bulan = bulan_indo.get(sekarang.strftime('%B'), sekarang.strftime('%B'))
    
    waktu_str = f"{nama_hari}, {sekarang.strftime('%d')} {nama_bulan} {sekarang.strftime('%Y')} | {sekarang.strftime('%H:%M:%S')} WIB"

    # Menambahkan Keterangan Waktu Akses
    pesan += "\n"
    pesan += f"🕒 <i>Waktu Akses: {waktu_str}</i>"
